...
class Accommodation(models.Model):
    ...
    def get_type_display(self) -> str:
        """Return the human-readable label for the `type` choice.

        This helper avoids static-analyser false-positives about
        generated `get_type_display` methods and provides a safe
        fallback when `type` is not set.
        """
...
class TourPackage(models.Model):
    ...
    def get_duration_display(self) -> str:
        """Return the human-readable label for the `duration` choice.

        Provides a safe fallback if `duration` is not set and avoids
        static-analyser false positives about Django's generated
        `get_FOO_display` helper methods.
        """
...
class Tourist(models.Model):
    ...
    def __str__(self):
        # Use getattr to avoid static-analyser complaints where `self.user`
        # may be treated as a model field descriptor instead of an instance.
...
class Booking(models.Model):
    ...
    def __str__(self):
        # Build a readable string without assuming related objects exist.
        # Use getattr with fallbacks to avoid AttributeError and to satisfy static analyzers.
        tourist = getattr(self, 'tourist', None)
        user = getattr(tourist, 'user', None) if tourist is not None else None
        tourist_username = getattr(user, 'username', '') or str(getattr(tourist, 'pk', '')) if tourist is not None else ''

        if self.booking_type == 'accommodation':
            accommodation = getattr(self, 'accommodation', None)
            acc_name = getattr(accommodation, 'name', '') or str(getattr(accommodation, 'pk', '')) if accommodation is not None else ''
            return f"จองที่พัก: {acc_name} - {tourist_username}" if (acc_name or tourist_username) else str(getattr(self, 'pk', ''))
        elif self.booking_type == 'tour_package':
            tour_package = getattr(self, 'tour_package', None)
            tp_name = getattr(tour_package, 'name', '') or str(getattr(tour_package, 'pk', '')) if tour_package is not None else ''
            return f"จองทัวร์: {tp_name} - {tourist_username}" if (tp_name or tourist_username) else str(getattr(self, 'pk', ''))
        # Fallback for unexpected booking_type values
        return f"การจอง: {str(getattr(self, 'pk', ''))} - {tourist_username}" if (getattr(self, 'pk', None) or tourist_username) else ''
